...
final class Legaltext
{
  ...
  public function restoreTocIds(): string
  {
    ...
    for ($node = $tocHeading->nextSibling; $node !== null; $node = $node->nextSibling) {
      // The generated ToC ends when the next content heading starts.
      ...
      foreach ($xpath->query('.//a[starts-with(@href, "#")]', $node) as $link) {
        // Only restore IDs that can safely be queried and written as HTML IDs.
    ...
    // Prefer exact text matches because they survive heading level changes.
    ...
    // Fall back to position for generated entries whose labels differ from the headings.
  ...
  private function createDocument(): ?DOMDocument
  {
    ...
    // Wrap the fragment so DOMDocument can parse and serialize only the original content.
